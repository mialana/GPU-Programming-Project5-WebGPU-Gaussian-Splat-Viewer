...
export function get_sorter(keysize: number, device: GPUDevice): SortStuff {
    ...
    const keys_count_adjusted =
        (Math.floor((keysize + keys_per_workgroup - 1) / keys_per_workgroup) +
            1) *
        keys_per_workgroup;
    ...
    const bind_group = device.createBindGroup({
        ...
        entries: [
            ...
            {
                binding: 2,
                resource: { buffer: ping_pong[0].sort_depths_buffer },
            },
            {
                binding: 3,
                resource: { buffer: ping_pong[1].sort_depths_buffer },
            },
            {
                binding: 4,
                resource: { buffer: ping_pong[0].sort_indices_buffer },
            },
            {
                binding: 5,
                resource: { buffer: ping_pong[1].sort_indices_buffer },
            },
        ],
    });
